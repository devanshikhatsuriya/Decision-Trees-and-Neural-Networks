...
def partial_accuracies(root_node, train_dataframe, test_dataframe, validation_dataframe):
    ...
    '''
        - does bfs traversal of decision tree and computes train, test and validation accuracies
            as the tree grows
        - generates a accuracy vs no. of nodes plot
    '''
    ...
    if root_node.is_empty:
        ...
        if 'no' not in counts:
            ...
            root_node.y_value = 'yes'
        elif 'yes' not in counts:
            bfs_tree.y_value = 'no'
            root_node.y_value = 'no'            
        else:
    ...
    root_node.test_df = test_dataframe
    root_node.val_df = validation_dataframe
        ...
        for i in range(len(dt_node.children)):
            ...
            if dt_node_child.is_empty:
                ...
                if 'no' not in counts:
                    ...
                    dt_node_child.y_value = 'yes'
                elif 'yes' not in counts:
                    node_child.y_value = 'no'
                    dt_node_child.y_value = 'no'
                else:
                    if counts['no'] >= counts['yes']:
                        node_child.y_value = 'no'
                        dt_node_child.y_value = 'no'
            ...
            if attr_category[node.split_attr] == 0:
                if i == 0:
                    ...
                    dt_node_child.test_df = node_child.test_df
                    dt_node_child.val_df = node_child.val_df
                else:
                    node_child.test_df = node.test_df[node.test_df[split_attr_name]>node.split_attr_values[0]]
                    node_child.val_df = node.val_df[node.val_df[split_attr_name]>node.split_attr_values[0]]
                    dt_node_child.test_df = node_child.test_df
...
# print("Two-way split using boolean one-hot representation for categorical attributes\n")
...
print("Splitting validation and test sets at each node...")
start = time.process_time()
...
def generate_leaves(node, l):
    if node.is_leaf:
        l.append(node)
    else:
        for child_node in node.children:
            generate_leaves(child_node, l)    


# In[42]:


def tree_size(node, size):
    size[0] += 1
    if not node.is_leaf:
        for child_node in node.children:
            tree_size(child_node, size)              
...
old_size = [0]
tree_size(root_node, old_size)


# In[44]:


def post_prune(root_node, train_dataframe, test_dataframe, validation_dataframe):
    
    '''
        does bfs traversal of decision tree and post prunes in a bottom up manner
    '''
    
    leaves = []
    generate_leaves(root_node, leaves)
    frontier = leaves[:]
    index = 0
    ...
    train_acc = 100 * numpy.count_nonzero(train_pred==train_dataframe['y'])/len(train_dataframe)
    test_acc = 100 * numpy.count_nonzero(test_pred==test_dataframe['y'])/len(test_dataframe)
    best_val_acc = 100 * numpy.count_nonzero(val_pred==val_dataframe['y'])/len(validation_dataframe)        

    num_nodes = numpy.append(num_nodes, index)
    train_accs = numpy.append(train_accs, train_acc)
    test_accs = numpy.append(test_accs, test_acc)
    val_accs = numpy.append(val_accs, best_val_acc)      
    ...
    while len(frontier) != 0:
        ...
        if leaf.parent != None:            
            new_frontier = []
            for other_leaf in frontier:
                if leaf.parent != other_leaf.parent:
                    new_frontier.append(other_leaf)
            frontier = new_frontier
            
            # prune this leaf, i.e. make leaf.parent a leaf
            new_leaves = []
            ...
            val_pred = numpy.empty(shape=(len(validation_dataframe)), dtype=object)
            for l in leaves:
                if not l.val_df.empty:
                    indices = numpy.array(l.val_df.index)
                    val_pred[indices] = l.y_value
            val_acc = 100 * numpy.count_nonzero(val_pred==val_dataframe['y'])/len(validation_dataframe) 
            
            # if better val accuracy, then keep this pruning
            if val_acc > best_val_acc:
                best_val_acc = val_acc
                train_pred = numpy.empty(shape=(len(train_dataframe)), dtype=object)
                test_pred = numpy.empty(shape=(len(test_dataframe)), dtype=object)
                for l in leaves:
                    if not l.dataframe.empty:
                        indices = numpy.array(l.dataframe.index)
                        train_pred[indices] = l.y_value
                    if not l.test_df.empty:
                        indices = numpy.array(l.test_df.index)
                        test_pred[indices] = l.y_value
            ...
            # else, undo this pruning
            else:
                leaf.parent.is_leaf = False
                leaves = store_leaves  
...
start = time.process_time()
acc = post_prune(root_node, train_df_new, test_df_new, val_df_new)
print(f"Time taken: {time.process_time()-start} seconds")

new_size = [0]
tree_size(root_node, new_size)
print(f"\nEarlier, no. of nodes in decision tree: {old_size[0]}")
print(f"No. of nodes after post-pruning: {new_size[0]}\n")  
...
plt.xlabel('No. of Times Pruned')
plt.ylabel('Accuracy (%)')

plt.legend()
plt.show()

fig_name = "1b_post_pruning.png"
